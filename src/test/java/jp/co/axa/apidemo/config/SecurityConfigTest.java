...
public class SecurityConfigTest {
    ...
    // We mock the service layer to isolate our tests to the security configuration.
    @MockBean
    private EmployeeService employeeService;

    // Test case for GET endpoint (permitAll)
    @Test
    @WithAnonymousUser // Simulate a request without any user authentication
    public void getEmployees_shouldSucceedWithNoAuthentication() throws Exception {
        mockMvc.perform(get("/api/v1/employees"))
                .andExpect(status().isOk());
    }

    // Test cases for POST endpoint (ADMIN role required)
    @Test
    ...
    // Test cases for PUT endpoint (ADMIN role required)
    @Test
    ...
    public void updateEmployee_shouldSucceed_WhenAuthenticatedAsAdmin() throws Exception {
        mockMvc.perform(put("/api/v1/employees/1")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"name\":\"updated\", \"salary\":60000, \"department\":\"IT\"}"))
                // We expect 404 here because the mocked service returns null, but the key is that we pass the security check (not 401 or 403).
                .andExpect(status().isNotFound());
    }

    // Test cases for DELETE endpoint (ADMIN role required)
    @Test
    @WithMockUser(username = "user", roles = {"USER"}) // Another way to simulate a user
